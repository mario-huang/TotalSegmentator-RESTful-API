...
async def segment_file(
    ...
    body: FileRequest = Depends(),
):
    print("body: " + str(body))
    return await add_timeout(input, asdict(body))


@app.post("/segment_url")
async def segment_url(body: UrlRequest):
    print("body: " + str(body))
    return await add_timeout(body.input, asdict(body.data))


async def add_timeout(input: str | UploadFile, body: dict[str, Any]):
    try:
...
async def process_segment(input: str | UploadFile, body: dict[str, Any]):
    ...
    try:
        ...
        output_img = totalsegmentator(input_img, None, **body)
